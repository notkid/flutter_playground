...
///
typedef void RenderStickyHeaderCallback(int stuckAmount, double stuckOffset);
...
class RenderStickyHeader extends RenderBox
  ...
  void performLayout() {
    ...
    if (_index == 0) {
      ...
      _postions[_index] = _postions[_index - 1] + headerHeight + contentHeight;
    }
    // report to widget how much the header is stuck.
    if (_callback != null) {
      if (stuckOffset < 0 && stuckOffset > -10) {
        // _callback(_index == 0 ? _index : _index - 1, stuckOffset);
      }
      if (stuckOffset > 0 && stuckOffset < 10) {
        _callback(_index, stuckOffset);
      }
  ...
  double determineStuckOffset() {
    ...
    if (scrollBox?.attached ?? false) {
      try {
        // print(localToGlobal(Offset.zero, ancestor: scrollBox).dy);
        // _postions[_index] = localToGlobal(Offset.zero, ancestor: scrollBox).dy;
